...
void RobotBox::goalStatusCallback(const actionlib_msgs::GoalStatusArray& msg)
{
    ...
    QString goalStatus = "Goal Status: " + QString::fromStdString(msg.status_list[i].text);
...
void RobotBox::displayBaseBattery(int battery)
{
    ...
    batString.sprintf("Base Battery: %.1fV/%.1fV", batVoltage, BAT_MAX);
